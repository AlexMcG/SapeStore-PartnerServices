...
/*
		@NamedQuery(name = "Book.findByBookTitle", query = "from Book b where b.bookTitle LIKE :bookTitle)"),
		@NamedQuery(name = "Book.findByBookAuthor", query = "from Book b where b.bookAuthor LIKE :bookAuthor)"),
		@NamedQuery(name = "Book.findByBookCategory", query = "from Book b where b.categoryId IN(Select categoryId from BookCategory where categoryName LIKE :categoryName)"), 

		@NamedQuery(name = "Book.findByBookISBN", query = "from Book b where b.isbn LIKE :isbn)"), */
		})
@NamedNativeQuery(name = "Book.SortByBookTitle", query = "SELECT * FROM SAPESTORE_BOOKS JOIN (SELECT SAPESTORE_BOOKS.ISBN, COUNT(BOOK_RATING_COMMENTS.ISBN) AS TOTAL_COMMENTS FROM SAPESTORE_BOOKS LEFT OUTER JOIN BOOK_RATING_COMMENTS ON BOOK_RATING_COMMENTS.ISBN = SAPESTORE_BOOKS.ISBN GROUP BY SAPESTORE_BOOKS.ISBN ORDER BY COUNT(BOOK_RATING_COMMENTS.ISBN)) COMMENTS ON SAPESTORE_BOOKS.ISBN = COMMENTS.ISBN WHERE BOOK_TITLE LIKE :bookTitle ORDER BY TOTAL_COMMENTS;", resultClass = Book.class)
public class Book implements Serializable{

	@Override
	public String toString() {
		return "Book [isbn=" + isbn + ", bookAuthor=" + bookAuthor + "]";
	}
